...
m:
    ...
    case 1:
        ...
        cout << "\t\t\t Password      \n";
        cin >> password;
        if (email == "r.antargangi@iitg.ac.in" && password == "Rohit@116")
        {
            administrator();
        }
...
void shopping::receipt()
{
    ...
    cout << "\nProduct No\tProd.name \t\t  quantity\t\t Price\t\tdiscount\t\t Amount with discount\n";
    ...
    for (int i = 0; i <= c; i++)
    {
        ...
        while (!data.eof())
        {
            ...
            {
                amount = price * arrq[i];
                double discount = amount - (amount * dis / 100);
                total += discount;
                cout << "\n"
                     << pcode << "\t\t" << pname <<"\t\t\t" << arrq[i] <<"\t\t\t" << price <<"\t\t\t" << dis <<"\t\t" << discount;
            }
            data >> pcode >> pname >> price >> dis;
        }
         data.close();
